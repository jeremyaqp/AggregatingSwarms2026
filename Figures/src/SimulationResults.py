...
def main():
    ...
    print("Showing simulations results from provided data : ")
    ...
    plt.plot(np.array(df_mean["ratio"]), np.array(df_mean["lit"]) / 64, color="red", linewidth=2, alpha=1.0)
    plt.plot(np.array(df_mean["ratio"]), np.array(df_mean_alignment["alignment"]), color="blue", linewidth=2, alpha=1.0)

    plt.axhline(y=0.18, color='black', linestyle='--')
    plt.xlim(-5.0, 5.0)
    plt.ylim(0, 1.0)
    ax.set_xlabel(r"$\epsilon / \tau_n$", fontsize=30)
    ax.set_ylabel(r"$N_\circ / N$", fontsize=30, color="darkred")
