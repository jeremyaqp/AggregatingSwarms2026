...
# PARAMETERS
# ---------------------------
CSV_GLOB = "../../Data/Raw/MSD_experiment.csv"   # path to your CSV
...
def main():
    ...
    for CSV_PATH in glob(CSV_GLOB):
        ...
        if mean_inside is not None:
            ax.plot(taus_common, mean_inside, color='red', linewidth=2.2,
        ...
        if mean_outside is not None:
            ax.plot(taus_common, mean_outside, color='blue', linewidth=2.2,
        ...
        plt.ylabel('MSD ($cm^2$)')  
        plt.tight_layout()
        OUTPUT_FIG = f"../out/msd_in_out.png"
